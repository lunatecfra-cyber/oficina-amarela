...
ALTER TABLE missions ADD COLUMN excluded_editor_id INTEGER REFERENCES users(id);
...
CREATE UNIQUE INDEX mission_assignment_active ON mission_assignments(mission_id) WHERE ended_at IS NULL;
CREATE INDEX mission_assignment_editor ON mission_assignments(editor_id, mission_id);
ALTER TABLE messages ADD COLUMN assignment_id INTEGER REFERENCES mission_assignments(id) ON DELETE SET NULL;
CREATE INDEX message_assignment ON messages(mission_id, assignment_id, created_at);

UPDATE missions SET first_delivered_at = created_at
WHERE delivery_link IS NOT NULL OR delivery_video_url IS NOT NULL OR status IN ('em_revisao', 'reedicao', 'aprovada', 'finalizada');
INSERT INTO mission_assignments(mission_id, editor_id, accepted_at)
SELECT id, reserved_by_id, COALESCE(reserved_at, created_at) FROM missions WHERE reserved_by_id IS NOT NULL;
-- Ambiguous legacy messages remain owner-only instead of leaking old assignments.
UPDATE messages SET assignment_id = (SELECT a.id FROM mission_assignments a JOIN missions p ON p.id = a.mission_id
  WHERE a.mission_id = messages.mission_id AND a.ended_at IS NULL AND p.reserved_at IS NOT NULL
  AND julianday(messages.created_at) >= julianday(a.accepted_at)
  AND (messages.author_id IN (a.editor_id,p.spokesperson_id) OR EXISTS(SELECT 1 FROM users u WHERE u.id = messages.author_id AND u.role = 'admin')));

CREATE TABLE mission_owner_events (
...
CREATE INDEX mission_owner_recipient ON mission_owner_events(previous_editor_id, created_at);
...
-- One event insert is the transaction boundary, including the outbox.
