...
const PARTNERS = [
  {
    name: "Discord",
    description: "Servidor pra conversar, tirar dúvidas e acompanhar o que tá rolando.",
  ...
  {
    name: "WhatsApp",
    description: "Grupo pra trocar ideia, pedir ajuda e ficar por dentro das novidades.",
...
export default async function PartnersPage() {
  ...
  return (
    <>
      {String(headerVariant) === "spokesperson" || String(headerVariant) === "porta-voz" ? (
        <AppHeaderSpokesperson />
      ) : String(headerVariant) === "inspector" || String(headerVariant) === "inspetor" ? (
        <AppHeaderInspector />
      ) : (
        <AppHeader />
      )}
      <main className="flex-1">
        <div className="mx-auto w-full max-w-5xl px-5 py-8 lg:px-8 lg:py-12">
          <div className="mb-8">
            <Link
              href={returnPath}
              className="tap-target mb-4 inline-flex text-sm text-muted hover:text-text"
            >
              ← Voltar
            </Link>
          ...
          <ul className="grid gap-4 sm:grid-cols-2">
            {PARTNERS.map((c) => {
              const isWsp = c.name.toLowerCase().includes("whatsapp");
              const brandColor = isWsp ? "#25D366" : "#5865F2";
              return (
                <li key={c.name} className="min-w-0">
                  <a
                    ...
                    style={{
                      ...
                      background: `linear-gradient(135deg, ${brandColor}12 0%, rgba(20,20,25,0.7) 100%)`,
                    }}
                  >
                    <div
