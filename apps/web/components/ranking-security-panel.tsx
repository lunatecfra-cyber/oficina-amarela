...
function invitationTone(status: string) {
  ...
  if (status === "usado" || status === "used")
    return "border-silver-lo/40 bg-surface-2 text-silver";
  if (status === "revogado" || status === "revoked")
...
export function RankingSecurityPanel() {
  ...
  const loadInvitations = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(data.error ?? data.erro ?? "Não foi possível carregar os convites.");
      setInvitations(data.invitations ?? data.convites ?? []);
    } catch (error) {
      setInvitationError(
        error instanceof Error ? error.message : "Não foi possível carregar os convites.",
      );
    } finally {
  ...
  const loadAudit = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error ?? data.erro ?? "Não foi possível carregar a auditoria.");
  ...
  async function postAction(body: Record<string, unknown>, endpoint = "/api/admin/ranking") {
    ...
    try {
      ...
      if (!res.ok) {
        setNotice({
          tone: "error",
          text: data.error ?? data.erro ?? "Não foi possível concluir a ação.",
        });
  ...
  async function copyLink() {
    try {
      ...
      setNotice({
        tone: "error",
        text: "Não foi possível copiar automaticamente. Selecione o link e copie.",
      });
    }
  }
  ...
  async function revokeInvitation(invitation: InvitationItem) {
    if (
      !window.confirm(
        `Revogar o convite enviado para ${invitation.email}? Esta ação não pode ser desfeita.`,
      )
    )
      return;
  ...
  return (
    <section className="mt-6">
      <div className="flex flex-col gap-4 border-b border-line pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Convites, correções e registros do ciclo eleitoral.
          </p>
        </div>
        <div
          className="flex overflow-x-auto rounded-lg border border-line bg-surface p-1"
          role="tablist"
          aria-label="Visualizações do painel"
        >
          {VIEWS.map((item) => (
            <button
              key={item.id}
      ...
      {notice && (
        <p
          role="status"
          className={`mt-4 border-l-2 px-3 py-2 text-sm ${notice.tone === "success" ? "border-ok text-ok" : "border-danger text-danger"}`}
        >
          {notice.text}
        </p>
      )}
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
          <section className="border border-line bg-surface/60 p-4">
            ...
            <p className="mt-1 text-xs leading-5 text-muted">
              Uso único, vinculado ao e-mail e válido por 7 dias.
            </p>
            <form className="mt-4 grid gap-3" onSubmit={createInvitation}>
              <label
                className="grid gap-1.5 text-xs font-medium text-muted"
              ...
              <button className="btn-gold w-full" type="submit" disabled={loadingAction}>
                {loadingAction ? "Gerando..." : "Gerar convite"}
              </button>
            </form>
            {link && (
              <div className="mt-5 border-t border-line-soft pt-4">
                ...
                <button type="button" className="btn-ghost mt-2 w-full" onClick={copyLink}>
                  Copiar link
                </button>
              </div>
            )}
          ...
          <section className="border border-line bg-surface/60">
            <div className="flex items-center justify-between border-b border-line px-4 py-3">
              ...
              <button
                type="button"
                className="tap-target -mr-3 text-xs text-muted hover:text-text"
            ...
            {loadingInvitations ? (
              <PanelState text="Carregando convites..." />
            ) : invitationError ? (
              <PanelState text={invitationError} error />
            ) : invitations.length === 0 ? (
              <PanelState text="Nenhum convite emitido ainda." />
            ) : (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-muted">
                          Expira em {formatDateTime(invitation.expires_at ?? invitation.expira_em)}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span
                          className={`rounded-full border px-2 py-1 text-[11px] font-medium ${invitationTone(status)}`}
                        >
                          {INVITATION_LABELS[status] ?? invitation.status}
                        </span>
                        {(status === "valido" || status === "valid") && (
                          <button
                            type="button"
        ...
        <section className="mt-5 border border-line bg-surface/60">
          <div className="flex flex-col gap-3 border-b border-line px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted">
                Ações administrativas registradas no ciclo.
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <select
                className="field-input !pl-3 min-h-11 py-1 text-sm"
                value={auditFilter}
              ...
              <button
                type="button"
                className="btn-ghost min-h-11 w-auto px-3 text-xs"
          ...
          {loadingAudit ? (
            <PanelState text="Carregando auditoria..." />
          ) : auditError ? (
            <PanelState text={auditError} error />
          ) : filteredAudit.length === 0 ? (
            <PanelState text="Nenhum registro para este filtro." />
          ) : (
            <ol className="divide-y divide-line-soft">
              {filteredAudit.map((log) => (
                <li
                  key={log.id}
                  className="grid gap-1 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:gap-x-5"
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-xs text-muted">
                      {log.ator_nome ?? "Sistema"} · {log.entidade} #{log.entidade_id}
                    </p>
                    {(log.detalhes?.motivo || log.detalhes?.reason) && (
                      <p className="mt-1 text-xs text-muted-2">
                        Motivo: {log.detalhes.motivo ?? log.detalhes.reason}
                      </p>
                    )}
                  </div>
                  <time className="text-xs text-muted-2" dateTime={log.criado_em}>
...
function PanelState({ text, error = false }: { text: string; error?: boolean }) {
  return (
    <p className={`px-4 py-8 text-center text-sm ${error ? "text-danger" : "text-muted"}`}>
      {text}
    </p>
  );
}
...
function OperationForm({
  ...
  return (
    <form
      className={`border p-4 ${dangerous ? "border-danger/40 bg-danger/[0.03]" : "border-line bg-surface/60"}`}
    ...
    >
      ...
      <p
        className={`mt-3 flex items-start gap-2 rounded-md border px-2.5 py-2 text-[11px] leading-5 ${dangerous ? "border-danger/40 bg-danger/[0.06] text-danger" : "border-line-soft text-muted-2"}`}
      >
        <svg
          viewBox="0 0 24 24"
          fill="none"
      ...
      <div className="mt-4 grid gap-3">
        <label className="grid gap-1.5 text-xs font-medium text-muted">
          {idLabel}
          <input name="id" type="number" min="1" required className="field-input" />
        </label>
        <label className="grid gap-1.5 text-xs font-medium text-muted">
          Motivo obrigatório
          <textarea
            ...
            placeholder="Registre a justificativa desta ação."
          />
        </label>
        {dangerous && (
          <label className="grid gap-1.5 text-xs font-medium text-danger">
            Confirmação forte: digite ANULAR
            <input
              value={confirmation}
        ...
        <button
          type="submit"
          className={
            dangerous ? "btn-ghost !border-danger/50 !text-danger hover:!bg-danger/10" : "btn-ghost"
          }
