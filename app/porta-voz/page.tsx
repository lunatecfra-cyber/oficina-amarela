...
export default async function SpokespersonHome() {
  ...
  const ordered = [...myMissions].sort((a, b) => {
    ...
    const byBucket =
      BUCKET_ORDER[spokespersonBucket(a.status)] -
      BUCKET_ORDER[spokespersonBucket(b.status)];
          ...
          <ul className="mt-6 flex flex-col gap-2.5">
            {ordered.map((p, n) => {
              ...
              const desiredDeadline =
                p.desiredDeadline ?? (p as any).prazoDesejado;
              ...
              const body = (
                <>
                  ...
                  <p className={`mt-1.5 text-sm font-medium ${msg.color}`}>
                    {msg.text}
                  </p>
