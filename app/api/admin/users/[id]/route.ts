...
export async function GET(
  ...
  if (!Number.isFinite(userId)) {
    return NextResponse.json({ error: "ID de usuário inválido.", erro: "ID inválido." }, { status: 400 });
  }

  const detail = await viewUserDetail(userId);
  if (!detail) return NextResponse.json({ error: "Conta não encontrada.", erro: "Não encontrada." }, { status: 404 });
